...
/**
 * Reject SELECTs that reference DuckDB system catalogs.
 * Throws `ValidationError` with `data.reason = 'system_catalog_access'`.
 */
  ...
  for (const pattern of FORBIDDEN_CATALOG_PATTERNS) {
    ...
    if (match) {
      throw validationError(`SQL references a denied system catalog: ${match[0]}.`, {
        ...
        catalog: match[0],
        recovery: {
          hint: 'Query only df_<id> tables. Use bls_dataframe_describe to list available dataframes.',
        },
      });
    }
  }
